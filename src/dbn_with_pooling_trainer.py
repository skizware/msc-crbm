import rbm_with_pooling_trainer
import numpy as np
...
class DbnTrainer(object):
    def __init__(self, dbn, train_set, valid_set, stats_collection_period=500, num_training_epochs=1,
                 output_directory='', image_loader=None, learning_rate_eval_period=25,
                 lr_error_multiplier=(1.) * 10 ** (-7)):
        self.rbm_trainers = []
        self.target_dbn = dbn
        ...
        for layer in self.target_dbn.layers:
            trainer = rbm_with_pooling_trainer.RbmTrainer(layer, train_set, valid_set, stats_collection_period, num_training_epochs,
                                             output_directory)
        ...
        for layer_idx in range(starting_layer, ending_layer + 1):
            ...
            for epoch_num in range(0, self.num_training_epochs):
                ...
                for image in self.train_set:
                    prev_layer = None
                    prev_layer_vis_sample = None
                    sample = image.copy()
                    if self.image_loader is not None:
                        sample = self.image_loader.load_image(sample)
                    if layer_idx != 0:
                        prev_layer_vis_sample = self.target_dbn.sample_h_given_v(sample, ending_layer=layer_idx - 1)[1]
                        sample = self.target_dbn.sample_h_given_v(prev_layer_vis_sample, starting_layer=layer_idx - 1,
                                                                  ending_layer=layer_idx)[3]
                        prev_layer = self.target_dbn.layers[layer_idx - 1]
                    hidden_bias_delta, sparsity_delta, \
                    visible_bias_delta, weight_group_delta = trainer.train_given_sample(sample, prev_layer,
                                                                                        prev_layer_vis_sample)
                    ...
                    if iteration_count % self.learning_rate_eval_period == 0:
                        ...
                        print (
                        "Setting learning rate to {}".format(self.lr_error_multiplier * layer_recreation_err_sqrd))
                        self.target_dbn.layers[layer_idx].set_learning_rate(
                            self.lr_error_multiplier * layer_recreation_err_sqrd)
                    ...
                    if (iteration_count % self.stats_collection_period == 0):
                        ...
                        trainer.collect_statistics(hidden_bias_delta, iteration_count, recreation_err_squared,
                                                   sparsity_delta, weight_group_delta, epoch_num, test_sample,
                                                   recreation,
                                                   visualized_filters)
    ...
    def visualize_filters(self, layer_idx):
        ...
        for i in range(0, num_bases):
            ...
            outp = self.target_dbn.sample_v_given_h(hidden_layer_input, starting_layer=layer_idx, filter_vis=True)
